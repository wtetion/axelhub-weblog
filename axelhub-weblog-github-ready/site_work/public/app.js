...
const I18N={
  en:{
    ...
    'graph.eyebrow':'GRAPH','graph.title':'Live Graph','graph.subtitle':'Speed, Money/s and Stolen Eggs • resets every 30 minutes.','graph.resetEvery':'30 MIN RESET','graph.speed':'SPEED','graph.moneyPerSecond':'MONEY / S','graph.stolen':'STOLEN EGGS'
  },
  th:{
...
function renderInventory(){
  ...
  $('inventoryGrid').innerHTML=filtered.length?filtered.map(it=>{const c=safeColor(rarityColor(it.rarity));return `<article class="egg-card inventory-browser-card" style="--rarity:${c}"><div class="egg-image"><img loading="lazy" src="${eggImage(it.name)}" onerror="this.onerror=null;this.src='/assets/egg-fallback.svg'"><span>x${fmt(it.count||1)}</span></div><b>${esc(it.name||t('common.unknownEgg'))}</b><div class="egg-bottom"><small class="egg-rarity" style="color:${c}">${esc(it.rarity||'Unknown')}</small>${it.mutation&&it.mutation!=='Normal'?`<em>${esc(it.mutation)}</em>`:''}</div>${it.finalValue!=null?`<div class="egg-final-value">FINAL VALUE <strong>${fmt(it.finalValue)}</strong></div>`:''}<small style="display:block;padding:0 9px 9px;color:#53667c;font-size:8px">${esc(it.displayName)}</small></article>`}).join(''):`<div class="inventory-empty-filter">${t('common.noMatchingEggs')}</div>`;
}
...
function drawLineChart(canvasId, points, key, label, unit, instant=false){
  ...
  const render=()=>{
    ...
    if(!vals.length || (points.length===0 && vals.every(v=>v===0))){ctx.fillStyle='rgba(108,125,148,.75)';ctx.fillText('Waiting for telemetry…',pad.l+15,pad.t+ch/2);ctx.fillStyle='rgba(108,125,148,.75)';ctx.font='10px Manrope, sans-serif';ctx.fillText(label,pad.l,h-8);ctx.textAlign='right';ctx.fillText(unit||'',w-pad.r,h-8);ctx.textAlign='left';return;}
...
async function refreshGraph(){
  try{
    ...
    const select=$('graphAccount'); if(select){const current=graphAccountFilter; const opts=['<option value="">All Accounts</option>'].concat((data.accounts||[]).map(a=>`<option value="${esc(a.account)}">${esc(a.displayName||a.account)}</option>`)); select.innerHTML=opts.join('');select.value=current;}
    ...
    const series=data.series||[];drawLineChart('speedChart',series,'speed','SPEED','studs/s');drawLineChart('moneyChart',series,'moneyPerSecond','MONEY / S','/s');drawLineChart('stolenChart',series,'eggsStolen','STOLEN EGGS','eggs');
...
window.addEventListener('resize',()=>{const s=graphData.series||[];drawLineChart('speedChart',s,'speed','SPEED','studs/s');drawLineChart('moneyChart',s,'moneyPerSecond','MONEY / S','/s');drawLineChart('stolenChart',s,'eggsStolen','STOLEN EGGS','eggs');});
...
document.querySelectorAll('.lang-btn').forEach(b=>b.addEventListener('click',()=>setLanguage(b.dataset.lang))); applyLanguage(); $('refresh').addEventListener('click',refreshAll);document.getElementById('logoutBtn')?.addEventListener('click',async()=>{try{await fetch('/api/auth/logout',{method:'POST',credentials:'same-origin'});}finally{location.href='/login.html';}});document.getElementById('copyTelemetryKey')?.addEventListener('click',()=>copyText(CURRENT_USER?.telemetryKey||'',document.getElementById('copyTelemetryKey')));document.addEventListener('click',async e=>{const job=e.target.closest('.copy-job');if(job){await copyText(job.dataset.jobId||'',job);return;}const inline=e.target.closest('.copy-inline');if(inline){await copyText(inline.dataset.copyValue||'',inline);return;}const row=e.target.closest('.copy-row');if(row){let value='';try{value=JSON.stringify(JSON.parse(decodeURIComponent(row.dataset.copyRow)),null,2);}catch{value=row.dataset.copyRow||'';}await copyText(value,row);return;}const del=e.target.closest('.delete-account');if(del){const account=decodeURIComponent(del.dataset.account||'');if(!account)return;if(!confirm(`Delete account "${account}" from this site?`))return;try{const r=await fetch(withSite('/api/accounts/'+encodeURIComponent(account)),{method:'DELETE',headers:{'x-axel-mutation-token':MUTATION_TOKEN}});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||r.status);await refreshAll();}catch(err){alert('Delete failed: '+err.message);}}});const stream=new EventSource(withSite('/api/stream'));stream.onmessage=()=>refreshAll();loadMutationToken().then(refreshAll);setInterval(refreshAll,10000);
